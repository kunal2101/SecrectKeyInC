...
class SavedNewsFragment :Fragment (R.layout.fragment_saved_news){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val itemTouchHelperCallback = object : ItemTouchHelper.SimpleCallback(
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
               val  position = viewHolder.adapterPosition
                val article = newsAdapter.difer.currentList[position]
                viewmodel.deleteArticle(article)
                Snackbar.make(view,"Sucessfully delete article",Snackbar.LENGTH_LONG).apply {
                    setAction("Undo"){
        ...
        viewmodel.getSavedNews().observe(viewLifecycleOwner, Observer { article ->
            newsAdapter.difer.submitList(article)
        })
